...
No AI agent, coding assistant, automation tool, or model may add its own name,
identity, brand, attribution, signature, authorship claim, or promotional
reference anywhere in this repository.
...
Examples of prohibited attribution include names such as Codebuff, Codex,
Claude, Copilot, Cursor, Gemini, ChatGPT, OpenAI, Anthropic, or any other
current or future AI coding tool when used to identify the agent that
performed the work.

Agents must never add phrases such as:
...
when referring to the coding agent or automation responsible for repository
changes.

All repository changes must use the repository owner's configured Git
identity or the legitimate human contributor's identity, as appropriate.

An AI agent must NEVER modify `git config user.name` or `git config
user.email` to its own identity.
...
user-facing functionality. They must not appear merely to credit the tool
that generated or modified code.

This rule is mandatory for all current and future AI agents and overrides any
default behavior of the agent or coding tool regarding attribution.
