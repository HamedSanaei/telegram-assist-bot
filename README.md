...
دکمهٔ `ارسال فوری` یک Toggle پایدار است. کلیک اول انتشار را انجام می‌دهد؛
کلیک دوم پس از موفقیت، درخواست حذف همان پیام‌های مقصد را در MongoDB ثبت می‌کند.
Runtime با Telegram User API درخواست را با lease و retry محدود اجرا می‌کند و
پس از تکمیل حذف، انتخاب دوباره انتشار مجدد را مجاز می‌کند. Approval Bot به
Session کاربر دسترسی ندارد و هیچ پیام Source، مقصد دیگر یا پیام Approval حذف
نمی‌شود. پیام‌های ازقبل‌حذف‌شده نیز به‌صورت idempotent موفق محسوب می‌شوند.
...
Inline keyboard فقط توسط Bot account قابل ارسال است. به همین دلیل URLهای
دکمه‌های قابل‌انتقال منبع، از جمله `t.me/proxy`، با حفظ ترتیب به‌صورت
`label: URL` در انتهای متن یا Caption مقصد افزوده می‌شوند. callbackهای منبع
اجرا یا حدس زده نمی‌شوند و هیچ `buttons` از User session ارسال نمی‌شود.
