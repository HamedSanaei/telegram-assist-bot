...
def test_rejects_ai_attribution_trailers(tmp_path: Path) -> None:
    """Co-author trailers naming an AI coding agent are rejected."""

    relative_path = _write_text(
        tmp_path,
        "trailer.md",
    ...
    assert IssueCode.PROHIBITED_ATTRIBUTION in {issue.code for issue in issues}
    assert issues[0].line == 2


def test_rejects_lowercase_ai_attribution_trailer(tmp_path: Path) -> None:
    """Attribution detection is case-insensitive for both trailer and tool."""
...
def test_rejects_ai_attribution_phrasing(tmp_path: Path, phrasing: str) -> None:
    """Every documented prohibited phrasing is rejected with tool name."""

    relative_path = _write_text(tmp_path, "attribution.txt", phrasing + "\n")
...
def test_accepts_human_co_author_trailer(tmp_path: Path) -> None:
    """Legitimate human contributors are not flagged by the policy."""
...
def test_accepts_placeholder_examples_in_policy_documentation(tmp_path: Path) -> None:
    """The AGENTS.md policy examples themselves remain compliant."""
...
def test_current_agents_policy_has_no_prohibited_attribution() -> None:
    """The policy document itself passes the scanner it defines."""
