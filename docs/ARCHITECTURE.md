...
| `AdvertisementRepository` | Snapshot و Slotهای پایدار تبلیغات |
| `AdvertisementReportRepository` | Projection محدود، read-only و مقصد-مجاز گزارش تبلیغات |
...
گزارش‌های `/ads_today`، `/ads_upcoming` و `/ads_failures` فقط با Config صریح
`advertisements.reports.enabled` ثبت می‌شوند. Handler پیش از Query، private chat،
فعال‌بودن مدیر، `approval.view` و مقصدهای مجاز را بررسی می‌کند. مرزهای تقویمی با
ZoneInfo تنظیم گزارش ساخته و به UTC تبدیل می‌شوند؛ MongoDB فقط Projection لازم را
با `max_items + 1` می‌خواند و Renderer مستقل از Bot SDK فقط آیتم کامل نمایش می‌دهد.
...
2. **قراردادهای داده‌ای ورودی و خروجی (Context & Output Schemas):**
   ...
   - برای خروجی هر تسک، یک کلاس پایدار مبتنی بر Pydantic تعریف شده است؛ Advertisement و Scoring نسخه ۱ باقی مانده‌اند و Semantic Duplicate و Categorization به‌طور صریح به نسخه ۲ ارتقا یافته‌اند:
     ...
     - `CategorizationOutput` نسخه ۲: شامل `category_id` (str)، `confidence` (float 0..1) و `reason` (str).
...
### جریان تثبیت‌شدهٔ فاز دوم (T054)

مرز واقعی فاز دوم به‌ترتیب Config typed، دریافت User API و Snapshot immutable، expansion
Slotهای UTC-aware، حل تداخل CAS، claim/Publication idempotent و گزارش read-only Bot API است.
Snapshot ID در Slot pin می‌شود؛ بنابراین edit منبع نسخهٔ قبلی را بازنویسی نمی‌کند. زمان اصلی
`due_at` برای هویت و audit ثابت می‌ماند و فقط `effective_due_at` حاصل collision را تعیین می‌کند.
Restart با بازسازی adapterها از همان MongoDB و Lease منقضی بازیابی می‌شود و unique indexها به‌همراه
CAS از Snapshot، Slot یا Publication تکراری جلوگیری می‌کنند. Worker تبلیغ همچنان seam ایزوله است و
فعال‌سازی composition/runtime تازه‌ای در T054 انجام نمی‌شود. ماتریس ۱۲‌ردیفی بخش ۱۷ مالک و شاهد
آزمون دقیق هر معیار را ثبت می‌کند.
...
12. رفتار عملیات Reject خارج از قرارداد موجود Approval هنوز تصمیم صریح ندارد؛ قرارداد گزارش تبلیغات در ADR-039 حل شده است.
...
### دسته‌بندی AI ایزولهٔ T044

T044 یک state افزایشی و مستقل `categorization_processing` روی Post دارد. use case
`CategorizeWithAI` ترتیب typed و صریح `ai`، `keyword` و `source_default` را عیناً
اجرا می‌کند؛ `source_default` دقیقاً یک‌بار و در انتهای ترتیب است. موفقیت یک روش،
روش‌های بعدی را متوقف می‌کند و keyword پیش از AI هیچ Cache read، AIJob، Guard یا
Provider activity ایجاد نمی‌کند. شکست یا Category نامعتبر AI فقط روش‌های پس از AI
را اجرا می‌کند و نتیجهٔ T018 با method واقعی `Keyword` یا `SourceDefault` باقی
می‌ماند.

خروجی Categorization به prompt `2.0.0` و schema `2` ارتقا یافته و شناسهٔ پایدار
`category_id` را برمی‌گرداند. تنها Category IDهای فعال و aliasهای دقیق پیکربندی
پذیرفته می‌شوند؛ display name هویت persistence نیست و fingerprint Taxonomy از
فهرست IDهای فعال و aliasهای مرتب‌شده با JSON قطعی UTF-8 و SHA-256 ساخته می‌شود.
نتیجه و state با CAS نسخهٔ پردازش ثبت می‌شوند و override دستی موجود یا هم‌زمان
همیشه بر completion AI مقدم است. mapper اسناد قدیمی فاقد state جدید را با
`NotRequested` نسخهٔ صفر می‌خواند. Header تأیید فقط metadata دسته‌بندی موجود را
به‌صورت افزایشی نمایش می‌دهد. T044 هیچ AI Worker، Runtime wiring، CLI یا Telegram
handler جدیدی ثبت نمی‌کند.

### امتیازدهی تأخیری ایزولهٔ T045

T045 یک state مستقل و افزایشی `scoring_processing` روی Post دارد. `ScheduleAIScoring`
فقط پس از boundary صریح `approval_ready` یا `approval_delivered` و با flag فعال، یک
AIJob منطقی `scoring` می‌سازد. هویت Job از Post، task، prompt `2.0.0` و schema `2`
قطعی است و `next_run_at` دقیقاً از
`source_published_at + configured delay_seconds` به‌صورت UTC-aware محاسبه می‌شود.
صف و index claim همان قرارداد T035 باقی مانده‌اند.

`ScoringHandler` پیش از ساخت ورودی Provider، lease و نسخهٔ Job و وجود Post را بررسی
می‌کند؛ Post منقضی یا حذف‌شده Provider را فراخوانی نمی‌کند و Job را stale/expired
می‌بندد. `ApplyAIScore` فقط خروجی validated شامل score صحیح ۰ تا ۱۰۰، confidence،
reason و componentهای اختیاری را با CAS ذخیره می‌کند. policyهای شکست فقط
`retry_later` و `mark_unavailable` هستند و هیچ score ساختگی یا gate انتشار ندارند.

پس از اولین persistence، `ApprovalScoringFanout` synchronizer موجود را با state تازه
Post و DestinationSelection اجرا می‌کند. refresh اجباری metadata، نسخهٔ انتخاب مقصد
