...
این سند طرح معماری پیش از شروع پیاده‌سازی است. منبع نیازمندی فعلی فایل `REQUIREMENTS.md` در ریشه مخزن است؛ مسیر `docs/REQUIREMENTS.md` که در دستورهای کاری ذکر شده فعلاً وجود ندارد. تا زمان تصمیم درباره جابه‌جایی سند، ارجاع‌های این مجموعه به مسیر واقعی انجام می‌شوند.
...
- **قابلیت‌های آینده و نیازمند بازتعریف:** فازهای سوم تا پنجم. این بخش‌ها در `REQUIREMENTS.md` پیشنهادی‌اند و پیش از پیاده‌سازی باید معیار پذیرش و اولویت محصولی پیدا کنند.
...
وضعیت‌های `REQUIREMENTS.md `10` نقطه شروع‌اند، اما پیش از پیاده‌سازی در T003 به Transitionهای مجاز و وضعیت مستقل هر مقصد تبدیل می‌شوند؛ یک Enum کلی به‌تنهایی برای انتشار جزئی چند مقصد کافی نیست.
...
- فایل Commitشدنی فقط `config/configuration.example.json` و فاقد مقدار حساس است.
- Configuration محلی از مسیر خارج از Git خوانده می‌شود؛ Secretها از Environment Variable یا Secret Manager تزریق می‌شوند.
- Config شامل MongoDB، Session path، Bot، Adminها، Source/Destinationها، Feature Flagها، Timezone، Logging، AI routing و Advertisementها است.
- Validator در Startup همه فیلدهای ضروری، ZoneInfo، مقادیر Enum، URLها، بازه‌ها، ارجاع Secret و سازگاری مقصدها را بررسی می‌کند.
- خطاها یک‌جا و با مسیر فیلد گزارش می‌شوند و برنامه پیش از اتصال خارجی متوقف می‌شود.
- Token، API Key، Mongo password، Session content و متن خام حساس در Log یا Exception نمایش داده نمی‌شوند.
...
1. سند نیازمندی در ریشه است، در حالی که Workflow مسیر `docs/REQUIREMENTS.md` را می‌خواهد.
2. SDKهای Telegram User API و Bot API و نسخه Python پشتیبانی‌شده تعیین نشده‌اند.
3. رفتار دکمه «فوری» هم Toggle انتخاب و هم آغاز فوری انتشار توصیف شده؛ وجود یا نبود مرحله Confirm باید روشن شود.
4. `minimum AI score` برای پیشنهاد کانال مبدا با امتیازدهی حداقل ۲۰ دقیقه بعد و امکان ارسال زودتر برای تأیید تعارض زمانی دارد.
5. Providerها، Modelها، روش Auth، Quota و Schema واقعی AI مشخص نشده‌اند.
6. بخش ۷ Fallback چندمدلی را آینده می‌نامد، اما بخش ۱۱ آن را با معیار پذیرش الزام‌آور می‌کند؛ این نقشه‌راه بخش ۱۱ را برای فاز اول ملاک گرفته است.
7. مقصد پیام تأیید می‌تواند گروه/کانال مشترک یا گفت‌وگوی جداگانه هر مدیر باشد؛ حالت‌های لازم و UX نهایی مشخص نیست.
8. سیاست پیش‌فرض شکست AI برای هر Task و مسیر دقیق «بررسی دستی» مشخص نشده است.
9. آستانه «بسیار نزدیک»، آستانه معنایی اولیه و رفتار Duplicate (رد یا دستی) مقدار قطعی ندارند.
10. Storage اولیه محیط Production (دیسک پایدار یا Object Storage)، سقف حجم Media و ظرفیت ۱۴روزه معلوم نیست.
11. رفتار با Edit/Delete پیام منبع و پیام‌های Forwardشده تعریف نشده است.
12. سیاست Collision تبلیغ و پست عادی گزینه‌ها را نام می‌برد ولی Default قطعی ندارد.
13. رفتار Cache تبلیغ پس از Edit منبع قابل تنظیم است، اما Default و بازه Refresh تعیین نشده‌اند.
14. سطح دسترسی/Roleهای Admin، Commandهای گزارش و عملیات Reject صریح تعریف نشده‌اند.
15. فازهای سوم تا پنجم پیشنهادی‌اند و معیار پذیرش، UX، داده و اولویت قطعی ندارند؛ T055 تا T057 ابتدا آن‌ها را قابل برنامه‌ریزی می‌کنند.

ابهام‌های 1 تا 15 در زمان فعال‌شدن Task مرتبط باید حل و در `docs/DECISIONS.md` یا Requirement اصلاح‌شده ثبت شوند؛ هیچ‌کدام مانع T001 نیست.
