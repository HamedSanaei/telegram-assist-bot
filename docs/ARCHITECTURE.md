...
مدل‌های `Publication`، `ScheduledPublication`، مدل‌های AI و Advertisement هنوز
فقط برای Taskهای صریح بعدی برنامه‌ریزی شده‌اند و قرارداد اجرایی ندارند.
...
immutable است؛ publication همچنان پیاده نشده است.
...
- `media_groups`: عضوهای idempotent و مرتب، deadlineهای quiet/max-wait و finalization اتمیک؛
...
Album با کلید canonical source + media-group ID ساخته می‌شود. deadlineهای quiet
و maximum-wait پایدارند، عضوهای replay/out-of-order مرتب می‌شوند و تنها یک
finalizer با CAS برنده است. عضو دیررس پیش از finalization پذیرفته و پس از آن
به‌صورت قطعی نادیده گرفته می‌شود؛ Media مستقل، Post مستقل باقی می‌ماند.
