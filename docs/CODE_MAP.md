...
واحد ingest/claim اتمیک و restart recovery را فراهم می‌کند. Media، AI، Bot API،
publication و Presentation handler هنوز پیاده نشده‌اند.
...
| `bootstrap/cli.py` | parsing امن `--config`، commandهای default/`login`/`ingest-text` و exit codeهای `0/2/3` |
